...
class Membrane(object):
    ...
    def fun_derenyi(self, x, y, p):
        """
        System of equations to solve for membrane shape. NO spontaneous curvature or coat. 
        Applied force is an additional parameter that is estimated via an
        additional boundary condition
        """
        ...
        fbar = p*(0.5*(1 - np.tanh(self.g*(x - self._aF))))/self._aF
    ...
    # def bc_derenyi(self, ya, yb, p):
    #     """
    #     Boundary condition with 2 extra parameters
    #     """
    #     ds  =1e-4
    #     return np.array([ya[0] - ds,  ya[1] - self._disp/self.R0, yb[1], ya[2], yb[2], ya[4],  yb[5] - self.lam0 * self.R0**2/ self.k])

    # def new_fun(self, x, y, p):
    #     """
    #     Function call for force with 2 extra parameters
    #     """

    #     b = 1
    #     ds = 1e-4
    #     c = 0.5*(p[1]*self.R0)*(1 - np.tanh(self.g*(x - self.a0)))
    #     dc = 0.5*(p[1]*self.R0)*self.g*(np.tanh(self.g*(x - self.a0))**2 - 1)
    #     #applied force

    #     fbar = p[0]*(0.5*((1 - np.tanh(self.g*(x - self.a0)))/self.a0 - (np.tanh(self.g*(x - self.aIn)) - np.tanh(self.g*(x - self.aOut)))/(self.aOut-self.aIn)))
    ...
    # def bc_new_fun(self, ya, yb, p):
    #     """
    #     Boundary condition with 2 extra parameters
    #     """
    #     ds  =1e-4
    #     return np.array([ya[0] - ds,  ya[1] - self._disp/self.R0, yb[1], ya[2], yb[2], ya[4], yb[4],  yb[5] - self.lam0 * self.R0**2/ self.k])
    ...
    # def loop_force_2_params(self, x, number_of_iterations, Z_spacing, initial, p_guess = None): 
    #     loop_sol, p_sol1, p_sol2 = [], [], []
    ...
    #             print(self._disp)
    #         else:
    #             self._disp = self._disp - Z_spacing
    #         print(self._disp)

    #         sol = self.compute(x, initial, np.array([0, self.c0]), type = 'new_fun')

    #         if sol.success == False:
    #             return loop_sol, p_sol1, p_sol2
    #             raise Exception('Failed')
    ...
    #             loop_sol = np.vstack((loop_sol, sol.sol(x))) 
    #         initial = sol.sol(x)
    #         p_guess = np.array([sol.p[0], sol.p[1]])

    #         p_sol1.append(sol.p[0] * (self.k/self.R0**3) * 2* np.pi * self.R0**2)
    #         p_sol2.append(sol.p[1])
    #     return loop_sol, p_sol1, p_sol2
